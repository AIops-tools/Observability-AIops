"""Encrypted secret storage for observability-aiops.

Secrets (Prometheus/Grafana bearer tokens) are NEVER written to disk in
plaintext. They live in ``~/.observability-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``OBSERVABILITY_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "observability-aiops"
CONFIG_DIR = Path.home() / ".observability-aiops"
MASTER_PASSWORD_ENV = "OBSERVABILITY_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``OBSERVABILITY_<TARGET>_TOKEN``; ``prefix='OBSERVABILITY_'``
    and ``suffix='_TOKEN'`` map them back to a target name. The plaintext
