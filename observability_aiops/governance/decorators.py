"""The ``@governed_tool`` decorator — mandatory wrapper for all observability-aiops MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.observability-aiops/audit.db``
...
from observability_aiops.governance.audit import detect_agent, get_engine
from observability_aiops.governance.budget import BudgetExceeded, get_budget
from observability_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from observability_aiops.governance.patterns import PatternMatch, get_pattern_engine
from observability_aiops.governance.policy import PolicyResult, get_policy_engine
from observability_aiops.governance.readonly import is_read_only, read_only_denial
from observability_aiops.governance.sanitize import sanitize

_log = logging.getLogger("observability-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all observability-aiops MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.observability-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("OBSERVABILITY_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("OBSERVABILITY_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (OBSERVABILITY_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set OBSERVABILITY_AUDIT_APPROVED_BY to the authorizing human (and "
            f"OBSERVABILITY_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`observability_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from observability_aiops.governance.undo import get_undo_store
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight observability calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``observability_aiops.ops.jobs`` → ``observability-aiops``
    ``mcp_server.server`` → ``observability-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("observability_aiops") or module.startswith("mcp_server"):
        return "observability-aiops"
    return "unknown"
