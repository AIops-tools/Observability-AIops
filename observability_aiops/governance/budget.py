...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``OBSERVABILITY_RUNAWAY_MAX`` times within ``OBSERVABILITY_RUNAWAY_WINDOW_SEC`` trips a
...
from observability_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing platform-specific; the decorator wraps it into a
