...
the inverse itself is destructive — tagged with its own risk tier). ``undo_apply``
is itself governed; its audit trail is built from the inner tool it calls.
...
def undo_list(limit: int = 50, target: Optional[str] = None) -> dict:
    ...
    length coincidence — when it is true, re-run with a higher ``limit``.
    ...
    probed = get_undo_store().list(status="recorded", limit=requested + 1)
    truncated = len(probed) > requested
    rows = probed[:requested]
    return {
        "count": len(rows),
        "returned": len(rows),
        "limit": requested,
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    recorded there too. Pass dry_run=True to preview the inverse call without
