...
``observability_aiops.governance.*`` and binds state to an isolated home via
``OBSERVABILITY_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import observability_aiops.governance.budget as budget_mod
import observability_aiops.governance.decorators as dec_mod
...
from observability_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OBSERVABILITY_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("OBSERVABILITY_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("OBSERVABILITY_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("OBSERVABILITY_POLICY_DISABLED", "1")
    _reset()
