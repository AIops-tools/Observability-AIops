...
nothing touches the real ``~/.observability-aiops``.
"""
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "OBSERVABILITY_PROM1_TOKEN=legacy-token\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("OBSERVABILITY_", "_TOKEN", "pw")
    assert imported == ["prom1"]
    assert ss.SecretStore.unlock("pw").get("prom1") == "legacy-token"
