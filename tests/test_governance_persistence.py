...
throwaway home (``OBSERVABILITY_AIOPS_HOME``) and assert that the rows
compliance evidence is built from actually land on disk, and that the
secure-by-default approver gate (no rules.yaml → high/critical needs an
approver) enforces.
"""
...
import observability_aiops.governance.audit as audit_mod
import observability_aiops.governance.policy as policy_mod
import observability_aiops.governance.undo as undo_mod
from observability_aiops.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OBSERVABILITY_AIOPS_HOME", str(tmp_path))
    monkeypatch.delenv("OBSERVABILITY_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("OBSERVABILITY_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-prom")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="prod-prom")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("OBSERVABILITY_AUDIT_APPROVED_BY", "sre-alice")
    result = _drop_widget(name="w3", target="prod-prom")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert len(audit) == 1
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "sre-alice"
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="prod-prom")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: delete_dashboard must
    capture the prior dashboard model before deleting and persist the
    recreate-undo on disk."""
    monkeypatch.setenv("OBSERVABILITY_AUDIT_APPROVED_BY", "sre-alice")
    prior = {"uid": "abc", "title": "Doomed", "version": 3}
    conn = MagicMock(name="conn")
    conn.target.platform = "grafana"
    conn.graf_get.return_value = {"dashboard": prior, "meta": {}}
    ...
    result = gov.delete_dashboard(uid="abc")
    assert result["priorState"]["dashboard"] == prior
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert "Doomed" in undo[0]["undo_params"]

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["delete_dashboard"]
