...
from observability_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_governed_write_is_refused_in_read_only(read_only):
    ...
    def delete_dashboard(target: str = "lab") -> dict:
        calls.append(target)
        return {"deleted": True}
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def firing_alerts(target: str = "lab") -> dict:
        return {"total": 0, "alerts": []}

    assert firing_alerts() == {"total": 0, "alerts": []}


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. create_silence) must be refused too."""

    @governed_tool(risk_level="medium")
    def create_silence(target: str = "lab") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "delete_dashboard" in dropped and "create_silence" in dropped
        assert "expire_silence" in dropped and "reload_prometheus_config" in dropped
        assert "firing_alerts" not in dropped and "instant_query" not in dropped
        ...
        assert "firing_alerts" in remaining, "reads must still be exposed"
        assert "loki_query" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
